...
float parsePress(uint8_t press_1, uint8_t press_2, uint8_t press_3, uint8_t press_4){
  uint32_t prs = ((uint32_t)press_1 << 24) | ((uint32_t)press_2 << 16) | ((uint32_t)press_3 << 8) | (uint32_t)press_4;
  /*
  Serial.println(press_1, BIN);
  Serial.println(press_2, BIN);
  Serial.println(press_3, BIN);
  Serial.println(press_4, BIN);
  Serial.println(prs, BIN);
  */
  float pressure = prs/100.;
...
void LoRaGet(){
  ...
  if(rx_valid){
    ...
    press = parsePress(pkt[8], pkt[9], pkt[10], pkt[11]);
  }
}
